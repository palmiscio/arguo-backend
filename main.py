...
async def call_mixtral(client, query):
    if not GROQ_KEY:
        return ModelResult(id="mixtral_groq", name="Mixtral", text="", ok=False, skip=True)
    t0 = time.time()
    try:
        r = await client.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {GROQ_KEY}", "Content-Type": "application/json"},
            json={"model": "mixtral-8x7b-32768", "max_tokens": 400,
                  "messages": [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": query}]},
            timeout=30
        )
        d = r.json()
        if "error" in d:
            raise Exception(str(d["error"]))
        text = d["choices"][0]["message"]["content"]
        return ModelResult(id="mixtral_groq", name="Mixtral", text=text, ok=True, latency_ms=int((time.time()-t0)*1000))
    except Exception as e:
        return ModelResult(id="mixtral_groq", name="Mixtral", text="", ok=False, error=str(e)[:100])


async def call_deepseek(client, query):
    if not GROQ_KEY:
        return ModelResult(id="deepseek", name="DeepSeek R1", text="", ok=False, skip=True)
    t0 = time.time()
    try:
        r = await client.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {GROQ_KEY}", "Content-Type": "application/json"},
            json={"model": "deepseek-r1-distill-llama-70b", "max_tokens": 400,
                  "messages": [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": query}]},
            timeout=30
        )
        d = r.json()
        ...
        return ModelResult(id="deepseek", name="DeepSeek R1", text=text, ok=True, latency_ms=int((time.time()-t0)*1000))
    except Exception as e:
        return ModelResult(id="deepseek", name="DeepSeek R1", text="", ok=False, error=str(e)[:100])
...
def root():
    ...
    if GROQ_KEY:       active.append("mixtral_groq")
    if GROQ_KEY:       active.append("deepseek")
...
async def analyze(req: QueryRequest):
    ...
    async with httpx.AsyncClient() as client:
        results = await asyncio.gather(
            ...
            call_mixtral(client, req.query),
            call_deepseek(client, req.query),
        )
