...
async def call_deepseek(client, query):
    if not GROQ_KEY:
        return ModelResult(id="deepseek", name="Qwen QwQ 32B", text="", ok=False, skip=True)
    t0 = time.time()
    try:
        r = await client.post(
            "https://api.groq.com/openai/v1/chat/completions",
        ...
        return ModelResult(id="deepseek", name="Qwen QwQ 32B", text=text, ok=True, latency_ms=int((time.time()-t0)*1000))
    except Exception as e:
        return ModelResult(id="deepseek", name="Qwen QwQ 32B", text="", ok=False, error=str(e)[:100])
