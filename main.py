...
async def synthesize(client, query, results):
    ...
    # Real consensus score based on keyword overlap
    keywords = ["breach", "terminate", "mutual", "clause", "law", "party", "damages",
                 "agreement", "notice", "performance", "impossible", "frustration"]
    overlap_scores = []
    for i in range(len(valid)):
        for j in range(i + 1, len(valid)):
            t1 = valid[i].text.lower()
            t2 = valid[j].text.lower()
            shared = sum(1 for k in keywords if k in t1 and k in t2)
            overlap_scores.append(shared / len(keywords))
    avg_overlap = sum(overlap_scores) / len(overlap_scores) if overlap_scores else 0.5
    score = int(50 + avg_overlap * 45)
