...
async def synthesize(client: httpx.AsyncClient, query: str, results: list[ModelResult]) -> dict:
    ...
    prompt = f"""Legal verifier. Analyze these AI responses. Return ONLY valid JSON, nothing else.

Q: {query}

{responses_text}

Return this JSON (complete all fields, never truncate any string):
{{"consensusScore":<0-100>,"consensusAnswer":"<max 2 sentences, same language as Q>","followUp1":"<complete question same language as Q>","followUp2":"<complete question same language as Q>","followUp3":"<complete question same language as Q>","divergences":"","modelAlignments":{{{align_keys}}}}}"""
    ...
    try:
        ...
        return {"consensusScore": 50, "consensusAnswer": valid[0].text[:300], "followUp1": "", "followUp2": "", "followUp3": "", "divergences": "", "modelAlignments": {}}
