...
async def draft_with_model(client, model_id, model_name, prompt, system_prompt):
    ...
    try:
        ...
        elif model_id in ("mistral", "groq_llama", "groq_gpt_oss", "groq_qwen"):
            model_map = {
                "mistral": ("https://api.mistral.ai/v1/chat/completions", MISTRAL_KEY, "mistral-large-latest", f"Bearer {MISTRAL_KEY}"),
                "groq_llama": ("https://api.groq.com/openai/v1/chat/completions", GROQ_KEY, "llama-3.3-70b-versatile", f"Bearer {GROQ_KEY}"),
                "groq_gpt_oss": ("https://api.groq.com/openai/v1/chat/completions", GROQ_KEY, "openai/gpt-oss-120b", f"Bearer {GROQ_KEY}"),
                "groq_qwen": ("https://api.groq.com/openai/v1/chat/completions", GROQ_KEY, "qwen/qwen3-32b", f"Bearer {GROQ_KEY}"),
            }
...
async def draft(req: DraftRequest):
    ...
    async with httpx.AsyncClient() as client:
        results = await asyncio.gather(
            ...
            draft_with_model(client, "groq_llama", "Llama 3 (Groq)", prompt, DRAFT_SYSTEM),
            draft_with_model(client, "groq_gpt_oss", "GPT-OSS 120B", prompt, DRAFT_SYSTEM),
            draft_with_model(client, "groq_qwen", "Qwen3 32B", prompt, DRAFT_SYSTEM),
        )
        results = list(results)
