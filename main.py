...
SYSTEM_PROMPT = """You are a specialized legal assistant with expertise in US law, EU law, Italian law, and international compliance.
Answer legal and compliance questions precisely, citing relevant normative sources (codes, laws, statutes, regulations, case law) when applicable.
Structure: direct answer → normative basis → exceptions or practical considerations.
Professional tone. Always respond in the same language as the question — if the question is in English, answer in English; if in Italian, answer in Italian; if in Spanish, answer in Spanish.
Always indicate the certainty level: high certainty / uncertainty / grey area."""
...
async def call_claude(client: httpx.AsyncClient, query: str) -> ModelResult:
    ...
    try:
        r = await client.post(
            ...
            json={"model": "claude-sonnet-4-20250514", "max_tokens": 1000, "system": SYSTEM_PROMPT,
...
async def call_mistral(client: httpx.AsyncClient, query: str) -> ModelResult:
    ...
    try:
        r = await client.post(
            ...
            json={"model": "mistral-large-latest", "max_tokens": 1000,
                  "messages": [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": query}]},
            timeout=30
        )
        d = r.json()
...
async def call_perplexity(client: httpx.AsyncClient, query: str) -> ModelResult:
    ...
    try:
        r = await client.post(
            ...
            json={"model": "llama-3.1-sonar-large-128k-online", "max_tokens": 1000,
                  "messages": [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": query}]},
            timeout=30
        )
        d = r.json()
