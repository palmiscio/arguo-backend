...
Be concise: maximum 3 sentences for the answer. No headers, no bullet lists, no markdown. Plain prose only.
Cite one key law or regulation naturally in the text if relevant.
...
async def synthesize(client: httpx.AsyncClient, query: str, results: list[ModelResult]) -> dict:
    ...
    prompt = f"""You are a legal verification engine. Analyze these {len(valid)} AI responses to the same legal question and return ONLY a valid JSON object, no other text.

QUESTION: {query}

{responses_text}

Rules for consensusAnswer:
- Maximum 2 sentences. Direct and complete. Never cut off mid-sentence.
- Same language as the question.
- After the answer, add exactly 3 follow-up questions on separate lines:
FOLLOW_UP_1: [question in same language as query]
FOLLOW_UP_2: [question in same language as query]
FOLLOW_UP_3: [question in same language as query]

JSON (no text outside the JSON):
{{"consensusScore":<0-100>,"consensusAnswer":"<2 sentence answer>\\nFOLLOW_UP_1: <q1>\\nFOLLOW_UP_2: <q2>\\nFOLLOW_UP_3: <q3>","divergences":"<main divergences or empty string>","modelAlignments":{{{align_keys}}}}}"""
