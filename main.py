...
async def synthesize(client: httpx.AsyncClient, query: str, results: list[ModelResult]) -> dict:
    ...
    if not valid:
        return {"consensusScore": 50, "consensusAnswer": "No response available.", "follow_up1": "What are the key legal requirements?", "follow_up2": "Are there any exceptions?", "follow_up3": "What remedies are available?", "divergences": "", "modelAlignments": {}}

    responses_text = "\n\n".join([f"=== {r.name} ===\n{r.text[:300]}" for r in valid])
    align_keys = ", ".join([f'"{r.name}": "agree"' for r in valid])

    # Single simple prompt - verdict only
    prompt = f"""You are a legal expert. Read these AI responses about: {query}

{responses_text}

Write a verdict of exactly 2 complete sentences summarizing the consensus. Same language as the question. Never cut off mid-sentence.
...
Verdict:"""
    ...
    try:
        ...
        verdict_text = r1.json()["content"][0]["text"].strip()
    except:
        verdict_text = valid[0].text[:300]

    # Follow-ups - completely separate simple prompt
    fup_prompt = f"""Question: {query}
Write 3 short follow-up questions (max 7 words each, same language). One per line, no numbering, no punctuation at start."""
    ...
    try:
        ...
        lines = [l.strip() for l in r2.json()["content"][0]["text"].strip().split("\n") if l.strip()]
        fup1 = lines[0] if len(lines) > 0 else "What are the notice requirements?"
        fup2 = lines[1] if len(lines) > 1 else "What remedies are available?"
        fup3 = lines[2] if len(lines) > 2 else "Can damages be claimed?"
    except:
        fup1 = "What are the notice requirements?"
        fup2 = "What remedies are available?"
        fup3 = "Can damages be claimed?"

    # Score - simple heuristic
    score = 75 if len(valid) >= 3 else 60

    return {
        "consensusScore": score,
        "consensusAnswer": verdict_text,
        "follow_up1": fup1,
        "follow_up2": fup2,
        "follow_up3": fup3,
        "divergences": "",
