...
async def call_claude(client, query):
    ...
    try:
        r = await client.post(
            ...
            json={"model": "claude-opus-4-5", "max_tokens": 600, "system": SYSTEM_PROMPT,
...
async def synthesize(client, query, results):
    ...
    raw_sources = [p["sources"] for p in parsed if p["sources"]]
...
Q3: [follow-up question max 8 words, same language]"""
    ...
    if low_count >= 2:
        ...
        warning = "⚠ One model flagged LOW confidence. Verify this applies to your specific jurisdiction."
    elif len(set([j for j in [p['jurisdiction'] for p in parsed] if j])) > 2:
        warning = "⚠ Models applied different jurisdictions. Confirm which legal system governs your situation."
    elif not sources:
