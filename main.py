...
If the request is a LEGAL QUESTION, follow this exact structure:
ANSWER: [1-2 sentences direct answer, same language as question]
JURISDICTION: [exact jurisdiction(s) applied]
SOURCES: [specific law, statute, article cited]
CONFIDENCE: [HIGH / MEDIUM / LOW] — [one sentence explaining why]

If the request is a DRAFTING request (contains words like "draft", "write", "create", "prepare"), produce a complete, ready-to-use document in the requested language and jurisdiction. Use proper legal formatting. Include all standard clauses. Then add:
JURISDICTION: [jurisdiction applied]
SOURCES: [key legal basis]
CONFIDENCE: [HIGH / MEDIUM / LOW] — [one sentence]

Never omit the structured fields at the end."""
...
async def call_gpt(client, query, jurisdiction_hint=""):
    ...
    try:
        ...
        d = r.json()
        if "error" in d: raise Exception(str(d["error"]))
        text = complete_sentence(d["choices"][0]["message"]["content"])
...
async def call_gpt_oss(client, query, jurisdiction_hint=""):
    ...
    try:
        ...
        if "error" in d: raise Exception(str(d["error"]))
        text = complete_sentence(d["choices"][0]["message"]["content"])
        return ModelResult(id="mixtral_groq", name="GPT-OSS 120B", text=text, ok=True, latency_ms=int((time.time()-t0)*1000))
    except Exception as e:
...
async def call_qwen(client, query, jurisdiction_hint=""):
    ...
    try:
        ...
        d = r.json()
        if "error" in d: raise Exception(str(d["error"]))
        raw = re.sub(r'<think>.*?</think>', '', d["choices"][0]["message"]["content"], flags=re.DOTALL).strip()
        text = complete_sentence(raw)
...
def complete_sentence(text: str) -> str:
    ...
    if last_end > len(text) * 0.5:  # Only if we keep at least half the text
        return text[:last_end+1]
    
    # Otherwise add a period if it ends mid-sentence
...
async def call_deepseek(client, query, jurisdiction_hint=""):
    if not DEEPSEEK_KEY:
        return ModelResult(id="deepseek_v3", name="DeepSeek V3", text="", ok=False, skip=True)
    text = ""
    t0 = time.time()
    try:
        r = await client.post(
            ...
            json={"model": "deepseek-chat", "max_tokens": 600,
                  "messages": [{"role": "system", "content": build_system_prompt(jurisdiction_hint)},
        ...
        if "error" in d: raise Exception(str(d["error"]))
        text = complete_sentence(d["choices"][0]["message"]["content"])
        return ModelResult(id="deepseek_v3", name="DeepSeek V3", text=text, ok=True, latency_ms=int((time.time()-t0)*1000))
    except Exception as e:
        return ModelResult(id="deepseek_v3", name="DeepSeek V3", text="", ok=False, error=str(e)[:100])
...
async def synthesize(client, query, results, selected_jurisdiction=""):
    ...
    prompt = f"""You are a senior legal verification engine. Analyze these AI responses to the same legal question. CRITICAL: You MUST write your entire response in the SAME language as the QUESTION above. If the question is in Italian, write everything in Italian. If in English, write in English. Never switch languages.
...
VERDICT: [3-4 precise sentences. Start with the direct answer. Then cite the key legal standard, statute or doctrine. Then note any important condition, exception or jurisdiction-specific nuance. Same language as question.]
SCORE: [integer 30-95. Calculate: agreement between answers 40% + confidence levels 30% + source consistency 30%. Be honest — penalize heavily if models disagree on substance.]
...
Q1: [follow-up question max 8 words. MUST be in the SAME language as the original question]
Q2: [follow-up question max 8 words. MUST be in the SAME language as the original question]
Q3: [follow-up question max 8 words. MUST be in the SAME language as the original question]"""
...
async def analyze(req: QueryRequest):
    ...
    async with httpx.AsyncClient() as client:
        ...
        results = await asyncio.gather(
            ...
            call_deepseek(client, req.query, jh),
        )
...
async def analyze_document(req: DocumentRequest):
    ...
    async with httpx.AsyncClient() as client:
        ...
        results = await asyncio.gather(
            ...
            call_deepseek(client, full_query, jh),
        )
        results = list(results)
        if not any(r.ok for r in results):
            raise HTTPException(status_code=503, detail="No model available")
...
DRAFT_SYSTEM = (
    "You are an expert legal drafter. Write complete, ready-to-use documents. "
    "When asked to draft something, produce the actual document immediately. "
    "Do not explain or describe what the document should contain. Just write it. "
    "Use proper formatting, salutations, and closings. "
    "After the document add: JURISDICTION: [applied] and CONFIDENCE: [HIGH/MEDIUM/LOW] - [reason]"
)
...
def build_draft_prompt(template: str, details: str, language: str, jurisdiction: str) -> str:
    lang_note = f"Write entirely in {language}." if language else "Match the language of the details."
    jur_note = f"Apply {jurisdiction} law." if jurisdiction else "Use the most appropriate jurisdiction."
    parts = [
        f"TASK: Draft a {template}",
        f"",
        f"KEY DETAILS:",
        details.strip(),
        f"",
        ...
        f"- {lang_note}",
        f"- {jur_note}",
        f"- Write the complete document. Start directly with the content.",
        f"- No preamble, no explanation. Just the document.",
        f"- After the document, on new lines add:",
        f"  JURISDICTION: [jurisdiction applied]",
        f"  CONFIDENCE: [HIGH/MEDIUM/LOW] - [one sentence reason]",
    ]
    return "\n".join(parts)


async def draft_with_model(client, model_id, model_name, prompt, system_prompt):
    t0 = time.time()
    try:
        if model_id == "claude":
            if not ANTHROPIC_KEY:
                return DraftResult(id=model_id, name=model_name, text="", ok=False, skip=True)
            r = await client.post(
                "https://api.anthropic.com/v1/messages",
                headers={"x-api-key": ANTHROPIC_KEY, "anthropic-version": "2023-06-01", "Content-Type": "application/json"},
                json={"model": "claude-opus-4-5", "max_tokens": 1500, "system": system_prompt,
                      "messages": [{"role": "user", "content": prompt}]},
                timeout=90
            )
            d = r.json()
        ...
        elif model_id in ("mistral", "groq", "mixtral_groq", "deepseek"):
            model_map = {
                "mistral": ("https://api.mistral.ai/v1/chat/completions", MISTRAL_KEY, "mistral-large-latest", f"Bearer {MISTRAL_KEY}"),
                "groq": ("https://api.groq.com/openai/v1/chat/completions", GROQ_KEY, "llama-3.3-70b-versatile", f"Bearer {GROQ_KEY}"),
                "mixtral_groq": ("https://api.groq.com/openai/v1/chat/completions", GROQ_KEY, "openai/gpt-oss-120b", f"Bearer {GROQ_KEY}"),
                "deepseek": ("https://api.groq.com/openai/v1/chat/completions", GROQ_KEY, "qwen/qwen3-32b", f"Bearer {GROQ_KEY}"),
            }
            url, key, model, auth = model_map[model_id]
            if not key:
                return DraftResult(id=model_id, name=model_name, text="", ok=False, skip=True)
            r = await client.post(
                url,
        ...
        text = complete_sentence(text)
        return DraftResult(id=model_id, name=model_name, text=text, ok=True, latency_ms=int((time.time()-t0)*1000))
    except Exception as e:
        return DraftResult(id=model_id, name=model_name, text="", ok=False, error=str(e)[:100])
...
async def draft(req: DraftRequest):
    ...
    prompt = build_draft_prompt(req.template, req.details, req.language, req.jurisdiction)

    async with httpx.AsyncClient() as client:
        results = await asyncio.gather(
            draft_with_model(client, "claude", "Claude", prompt, DRAFT_SYSTEM),
            draft_with_model(client, "gemini", "Gemini", prompt, DRAFT_SYSTEM),
            draft_with_model(client, "mistral", "Mistral", prompt, DRAFT_SYSTEM),
            draft_with_model(client, "groq", "Llama 3 (Groq)", prompt, DRAFT_SYSTEM),
            draft_with_model(client, "mixtral_groq", "GPT-OSS 120B", prompt, DRAFT_SYSTEM),
            draft_with_model(client, "deepseek", "Qwen3 32B", prompt, DRAFT_SYSTEM),
        )
        results = list(results)
    ...
    # Best draft = longest clean response
    best_raw = max(valid, key=lambda r: len(r.text)).text
    sep = "\n"
    clean_lines = [l for l in best_raw.split(sep) if not l.startswith("JURISDICTION:") and not l.startswith("CONFIDENCE:")]
    best_draft = sep.join(clean_lines).strip()

    # Parse confidence + jurisdictions
    confidence_flags = []
    jurisdictions = []
    for r in valid:
        p = parse_structured(r.text)
        if p.get("jurisdiction"):
            jurisdictions.append(p["jurisdiction"])
        confidence_flags.append({"model": r.name, "level": p.get("confidence", "HIGH"), "reason": p.get("confidence_reason", "")})

    # Follow-up suggestions
    q1 = "Add more specific clauses?"
    q2 = "Adjust the tone?"
    q3 = "Translate to another language?"

    if ANTHROPIC_KEY and best_draft:
        try:
            fup_prompt = (
                "Based on this drafted document, suggest 3 very short follow-up actions (max 6 words each). "
                "Same language as the document. Reply only Q1: Q2: Q3:\n\n" + best_draft[:300]
            )
            async with httpx.AsyncClient() as client:
                r = await client.post(
                    "https://api.anthropic.com/v1/messages",
                    ...
                    json={"model": "claude-opus-4-5", "max_tokens": 100,
                          "messages": [{"role": "user", "content": fup_prompt}]},
                    timeout=15
                )
                text = r.json()["content"][0]["text"].strip()
                for line in text.split("\n"):
                    line = line.strip()
                    if line.startswith("Q1:"): q1 = line[3:].strip()
                    elif line.startswith("Q2:"): q2 = line[3:].strip()
                    elif line.startswith("Q3:"): q3 = line[3:].strip()
        except Exception:
            pass

    score = min(95, 60 + len(valid) * 7)
